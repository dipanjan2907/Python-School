...
if con.is_connected():
    ...
    # cursor.execute("CREATE TABLE SALES (c_id INT PRIMARY KEY,c_name VARCHAR(40), p_id INT , Product_Name VARCHAR(500), Quantity_Sold INT,Total_Price FLOAT)")
    ...
    def check_customer_exists(cname):
        query = "SELECT 1 FROM Customer WHERE cname = ? LIMIT 1;"
    ...
    def view_sales(): # Display table SALES
        ...
        results = cursor.fetchall()
        print("-------------------------------SALES-------------------------------")
        print(f"{'C_ID':<10} {'C_NAME':<40} {'P_ID':<10} {'Name':<40} {'Sold_Quantity':<15} {'TOTAL_PRICE (₹)':<10}")
        print()
        for row in results:
            print(f"{row[0]:<10} {row[1]:<40} {row[2]:<10} {row[3]:<40} {row[4]:<15} {row[5]:<10}")
